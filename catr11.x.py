...
class CatR11OmniEngine:
    ...
    def next_dialect(self):
        with self._dialect_lock:
            dialect = self.dialects[self._dialect_index % len(self.dialects)]
            self._dialect_index += 1
        return dialect
    ...
    def generate(self, prompt, target_lang, simulate_latency=True):
        p = prompt.lower()
        dialect = self.next_dialect()
        ...
        for _ in range(3):
            dummy_input = self.bitnet_matmul(dummy_input)
        if simulate_latency:
            time.sleep(1.2)
...
class CatR1_1_X:
    ...
    def _run_inference(self, prompt, emit_thoughts=True, sleep_between_steps=True, simulate_latency=True):
        ...
        result = self.engine.generate(prompt, target_lang, simulate_latency=simulate_latency)
        return {
